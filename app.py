...
# Container pour l'affichage persistant
conteneur_affichage = st.empty()

if 'cours_genere' in st.session_state and st.session_state['cours_genere']:
    conteneur_affichage.markdown(f"<div class='success-box'>{st.session_state['cours_genere']}</div>", unsafe_allow_html=True)

if st.button("🚀 Générer le cours sur mesure"):
    # Vider le conteneur persistant si une nouvelle génération est lancée
    if 'cours_genere' in st.session_state:
        conteneur_affichage.empty()
        ...
        with st.spinner(f"🔄 Préparation d'une séance de {duree_seance} en cours..."):
            try:
                ...
                **Question X : [Texte]**
                ...
                # Sauvegarder dans le session_state une fois terminé
