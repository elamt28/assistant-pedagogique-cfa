...
if 'vient_de_generer' not in st.session_state:
    st.session_state['vient_de_generer'] = False
...
if st.button("🚀 Générer le cours sur mesure"):
    ...
    if not api_key:
        ...
        st.session_state['vient_de_generer'] = True
...
# Afficher le cours sauvegardé si on ne vient pas juste de le générer
if st.session_state['cours_genere'] != "" and not st.session_state['vient_de_generer']:
    st.markdown(f"<div class='success-box'>{st.session_state['cours_genere']}</div>", unsafe_allow_html=True)

if st.session_state['cours_genere'] != "":
    st.download_button(
        label="📥 Télécharger le cours (Markdown)",
        data=st.session_state['cours_genere'],
        file_name="cours_sur_mesure.md",
        mime="text/markdown"
    )
